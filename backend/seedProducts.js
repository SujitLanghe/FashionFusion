...
// Path to frontend assets images
...
// All 52 products from assets.js
const productsRaw = [
  {
...
async function seed() {
  try {
    ...
    if (existingCount > 0) {
      console.log(
        `⚠️  Database already has ${existingCount} products. Clearing them first...`
      );
      await productModel.deleteMany({});
      console.log("✅ Cleared existing products");
    }

    console.log(`\n📦 Seeding ${productsRaw.length} products...\n`);

    for (let i = 0; i < productsRaw.length; i++) {
      const product = productsRaw[i];
      console.log(
        `[${i + 1}/${productsRaw.length}] Uploading images for: ${product.name}`
      );
